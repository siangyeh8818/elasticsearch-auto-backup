...
def backup_index(source_ip ,source_port ,index):
    ...
    logger.debug("source_ip = %s"%source_ip)
    logger.debug("source_port = %s"%source_port)
    logger.debug("index = %s"%index)
#    os.system("touch /elasticsearch-backup/%s.json"%index)
    os.system("elasticdump  --input=http://%s:%s/%s --output=/elasticsearch-backup/%s.json"%(source_ip,source_port,index,index))
    logger.info("Finishing to dump elasticsearch data(index) , index : %s"%index)
    logger.info("dump data was store in /elasticsearch-backup/%s.json"%index)
...
def delete_index(source_ip ,source_port ,index):
    ...
    os.system("sed -i 's/<INDEX_NAME>/%s/g' /curator/delete_index_copy.yaml"%index)
    os.system("sed -i 's/<IP_ADDRESS>/%s/g' /curator/curator_copy.yaml"%source_ip)
    os.system("sed -i 's/<ES_PORT>/%s/g' /curator/curator_copy.yaml"%source_port)
    os.system("curator --config /curator/curator_copy.yaml /curator/delete_index_copy.yaml" )
    logger.info("Finishing to remove elasticsearch index , index : %s"%index)

now = datetime.datetime.now()
logger.info("Running time : %s"%now)

now2 = datetime.date.today()
#print "now2 : " + str(now2)
logger.info("Running date : %s"%now2)
